...
import { findBlock, plaintextOf } from '@shared-packages/doc-model';
...
describe('N4 table editor', () => {
	...
	it('DocEditor: one host, no <table>, overlay in the gutter, cross-cell select, no cell CE', async () => {
		let state = createEditorState(grid());
	...
	it('DocEditor onDispatch-only: Tab moves caret; last-row Enter keeps the column', async () => {
	...
	it('onDispatch-only last-cell Tab lands on the first cell of the new row', async () => {
		...
		const { container, rerender, unmount } = render(DocEditor, {
