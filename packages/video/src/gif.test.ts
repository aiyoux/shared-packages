...
describe('encodeGif', () => {
	it('writes one frame per grid slot with cumulative-error centisecond delays', async () => {
		...
		const delays = gifCalls.frames.map((f) => f.options?.delay);
		expect(delays.every((d) => typeof d === 'number' && d >= 3 && d <= 4)).toBe(true);
		expect(delays.reduce((a, b) => (a as number) + (b as number), 0)).toBe(100); // 30 frames × 33.33ms → exactly 100cs
	...
	it('keeps the frame overlapping the cut (ceil) and clamps delays to 2cs', async () => {
		...
		expect(gifCalls.frames.every((f) => f.options?.delay === 2)).toBe(true);
	...
	it('maps 24fps without drift and finishes the encoder', async () => {
		...
		expect(delays.reduce((a, b) => a + b, 0)).toBe(100); // 24 frames over exactly 1s
		expect(delays.some((d) => d === 4)).toBe(true);
	});
});