...
This repository contains the packages that are genuinely shared across app and module repositories — a UI component library, not app-specific business logic:
...
`module-sdk`, `item-tree`, and `shell-core` used to live here but were modular-app-specific (a SurrealDB sync engine, offline cache, domain schedulers, the record model, domain UI editors) — they've moved into `modular-app` as first-party workspace packages (`repos/module-sdk`, `repos/item-tree`, `repos/shell-core`). Nothing app-specific belongs in this repo; if a future addition isn't a generic, reusable UI primitive or a type/helper that primitive genuinely needs, it doesn't belong here.
